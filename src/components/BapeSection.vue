...
const { isLoading, hasError, retryLoad } = useImageLoader('/images/bape.png')

const sectionRef = ref<HTMLElement>()

const handleClick = () => {
  emit('setActiveSection', 'bape')
}
...
const isExpanded = () => props.activeSection === 'bape'
const isShrunk = () => props.activeSection && props.activeSection !== 'bape'
</script>
...
<template>
  <section
    ...
    :aria-label="`Bape sneaker section ${isExpanded() ? 'expanded' : 'collapsed'}`"
    aria-describedby="bape-description"
  >
    <div id="bape-description" class="sr-only">
      Bape sneaker showcase.
      {{
    ...
    <div class="flex flex-col items-center justify-center h-full relative">
      <h2 class="sneaker-title-bg" aria-hidden="true">BAPE</h2>
      ...
      <div class="sneaker-image-container">
        ...
        <div
          ...
          role="alert"
          aria-label="Failed to load Bape image"
        >
          ...
          <button
            ...
            aria-label="Retry loading Bape image"
          >
            Retry
          </button>
        </div>
        ...
        <img
          v-else
          src="/images/bape.png"
          alt="Bape sneaker in beige and brown colorway"
          class="sneaker-image"
      ...
      <h2 class="sneaker-title-front" :aria-hidden="isExpanded() ? 'false' : 'true'">BAPE</h2>
    </div>
  </section>
</template>
...
@media (min-width: 360px) and (max-width: 639px) {
  ...
  .sneaker-section.expanded .sneaker-image {
    transform: scale(1.6) rotate(8deg) translateY(-3%);
    max-width: 320px;
    max-height: 192px;
    margin: 0 auto;
  }
